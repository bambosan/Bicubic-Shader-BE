...
vec3 illum(vec3 diff,vec3 nv,vec3 lsc,float lmb,bool waterd){
	...
	if(!waterd)smap = mix(smap,0.,smoothstep(.6,.3,color.g));
...
vec4 inColor = color;
	...
	if(waterd){ n = calcnw(nv); wflag = 0.5; }
#endif
	...
	diffuse.rgb = illum(diffuse.rgb,n,lsc,lmb,waterd);
	...
	diffuse.rgb = mix(diffuse.rgb,csc(fresnel),(fresnel*rain*nv.y)*.2*smoothstep(.845,.87,uv1.y));
#endif
